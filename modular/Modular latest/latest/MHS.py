...
class Controller(Process):
    ...
    def initComponents(self):
        self.CallGen = [N.CallGen for N in Env.Env.Environment.MHS_nodes if N.ID==self.ID][0]
        self.ALE_Controller = [N.Controller for N in Env.Env.Environment.ALE_nodes if N.ID==self.ID][0]
        ...
        while True:
            ...
            if self.eventsFired[0] == self.ALE_Incoming and self.Mode == Mode.CallGen: #Valid Incoming Call
                ...
                if self.eventsFired[0] == self.ALE_Linked:
                    ...
                    self.Mode = Mode.CallGen
            elif self.eventsFired[0] == self.ALE_Linked and self.Mode == Mode.InCall:
...
class CallGen(Process):
    ...
    def initComponents(self):
        self.Controller = [N.Controller for N in Env.Env.Environment.MHS_nodes if N.ID==self.ID][0]
        self.ALE_Controller = [N.Controller for N in Env.Env.Environment.ALE_nodes if N.ID==self.ID][0]
        self.ALE_Mode = [N.Mode for N in Env.Env.Environment.ALE_nodes if N.ID==self.ID][0]
        ...
        while True:
            ...
            if self.Controller.Mode == Mode.CallGen and self.ALE_Controller.Mode == self.ALE_Mode.Scanning: #Check that it is still valid to send a call
                ...
                while targetNodeID == self.ID or targetNodeID == -1: #Mini function to make sure
                    targetNodeID = random.randint(0,Env.G.num_nodes-1)  #target isn't itself
...
class Modem_Rx(Process): #Can move these to new module if needed
    ...
    def execute(self):
        if self.Controller.Mode == Mode.Receving: #Check in correct Mode
            yield (get, self, Env.Env.Environment.grid[self.linked_data.chan][self.ID]),(hold, self, Env.G.linked_timeout)
            if not self.acquired(Env.Env.Environment.grid[self.linked_data.cha][self.ID]): #Nothing to report
...
class Modem_Tx(Process): #Can move these to new module if needed
    ...
    def execute(self):
        yield hold, self, random.uniform(0,60)
        if self.Controller.Mode == Mode.TrafficGen: #Check it's still in the correct Mode
            message = Functions.createMessage(self.linked_data)
            for each_station in Env.Env.Environment.grid[message.chan]: #This code is same as Linked_Rx code
            ...
            for each_station in Env.Env.Environment.grid[message.chan]:
            ...
            if random.random() < Env.Env.G.chance_reply:
...
class Functions():
    def createMessage(self, data):
        ...
        returnData
        Env.Env.Environment.updateLQA(returnData)
        return returnData
