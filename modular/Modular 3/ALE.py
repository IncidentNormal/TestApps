    ...
    def execute(self):
        while True:
            ...
            if self.eventsFired[0] == self.MHS_Incoming and self.Mode == Mode.Scanning:
                ...
                linking_data = self.eventsFired[0].signalParam
                MHS.Controller.ALE_Incoming.signal(linking_data)
                ...
                MHS.Controller.ALE_Linked.signal(linked_data)
...
class Scanning(Process):
    ...
    def execute(self):
        ...
        while True:
            yield (get, self, Env.Environment.grid[self.current_channel][self.ID], forMe),(hold, self, Env.G.dwell_time)
            if not self.acquired(Env.Environment.grid[self.scan_chan][self.ID]): #Nothing to report
                if self.scan_chan==Env.G.num_chans-1: self.scan_chan = 0 #Increment channel or set to 0 if at end of cycle
                else: self.scan_chan+=1
            else: #Received something
                info = self.got[0]
                if info.type == Env.dataType.CALL and ALE.Controller.Mode == ALE.Mode.Scanning: #Check it hasn't changed mode since start of dwell time
                    ALE.Controller.ScanningCall.signal(info)
                    yield passivate, self
                else:
                    pass 
            if ALE.Controller.Mode != ALE.Mode.Scanning: #At end of channel scan, check if still in Scanning mode
                yield passivate, self
...
class Linking_Tx(Process):
    ...
    def execute(self):
        ...
        for channel in chanOrder: #try each one in turn
            if Env.Environment.grid[channel][self.ID].nrBuffered==0:
                call.chan = channel
                call = Functions.updateLQA(call)
                for each_station in Env.Environment.grid[call.chan]: #filling channel with call
                    yield put, self, each_station, [call]
                break
        yield hold, self, call.signal_time
        for each_station in Env.Environment.grid[call.chan]:
            if each_station.nrBuffered==1: # Should have been taken by destination
                yield get, self, each_station, 1
        yield hold, self, 0.0001
        yield (get, self, Env.Environment.grid[call.chan][self.ID], 1),(hold, self, Env.G.response_timeout)
        if not self.acquired(Env.Environment.grid[call.chan][self.ID]):
            ...
            for each_station in Env.Environment.grid[ack_data.chan]:
...
class Linking_Rx(Process):
    ...
    def execute(self):
        ...
        for each_station in Env.Environment.grid[self.linking_data.chan]:
        ...
        for each_station in Env.Environment.grid[self.chan]:
            if each_station.nrBuffered==1:
                yield get, self, each_station, 1
        yield hold, self, 0.0001 #-----------------Hold for nominal time to prevent SimPy tie
        yield (get, self, Env.Environment.grid[self.linking_data.chan][self.ID], 1),(hold, self, Env.G.response_timeout)
        if not self.acquired(Env.Environment.grid[self.linking_data.chan][self.ID]): #Did not receive ACK
            ALE.Controller.TimedOut.signal()
        else: #Received ACK
            linking_data = self.got[0]
            yield hold, self, linking_data.signal_time
            ALE.Controller.Linked.signal(linking_data)
